...
- Keep durable ownership persistence in `BatteryControlOwnership`; keep pure tuple matching and state mapping in `ChargeReconciliationPolicy`. Hardware reads, task ownership, and published state remain in `ChargeController`.
