...
- Keep the POSIX ownership journal implementation in `BatteryControlOwnershipJournal`; `UserSettings` retains ownership loading, transitions, and persistence errors. Keep tuple matching and observed-state interpretation in `ChargeReconciliationPolicy`; state presentation remains in `ChargeState`. Hardware reads, task ownership, and published state remain in `ChargeController`.
