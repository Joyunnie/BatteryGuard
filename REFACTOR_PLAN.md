...
18. `[hostile review 보완 및 자동 검증 완료]` background 비용 축소와 안전 heartbeat 분리: routine diagnostics는 30초 단위로 묶고 safety/control/lifecycle/failure는 즉시 flush하며, 정상 종료 직전 pending routine context도 저장한다. IOKit 알림을 100ms 동안 coalesce해 주 측정 경로로 사용하고 30초 watchdog을 유지하되, controller에는 charge/power/temperature만 담은 control measurement를 전달한다. 동일 snapshot 억제와 무관하게 Top Up/Discharge는 active-mode 2초 liveness heartbeat를, verified-limit 이력은 15분 one-shot heartbeat를 사용하며 readiness 완료 직후 최신 snapshot을 한 번 처리한다. 독립 센서인 SMC는 IOKit 값으로 늦추지 않고 Heat Protection 동안 5초 상한을 유지하고, display freshness와 sampling due 경계를 분리해 정확히 5초부터 재측정한다. exact Maintain worker는 bounded `pgrep -fl` 후보 조회 뒤 `identity-before -> ps argv -> identity-after`가 모두 일치할 때만 인정한다. 따라서 안전 검증을 약화했던 SMC 분당 12회 및 reconciliation 2-process 예상치는 철회한다. 현재 코드 경로상 예상 개선은 routine diagnostics 파일 교체 최대 분당 약 36회에서 2회, battery watchdog polling 분당 30회에서 2회이며, 실제 wakeup·subprocess·write 수치는 runtime profile 전에는 완료 수치로 주장하지 않는다. Runner의 일반 20ms poll과 teardown 전용 2ms poll 분리는 PR #15 기준선에서 이미 적용된 상태를 유지한다. 엄격 동시성·경고 오류화 조건의 246개 테스트, Release 빌드, Analyze를 모두 통과했다.
