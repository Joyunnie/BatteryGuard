...
### 0.26 Maintain worker 후보 상한 수정과 성능 재측정 (2026-08-09)

PR #17 이후 성능 비교에서 일부 앱 실행이 초기화에 실패했는데도 짧은 수명 때문에 CPU·wakeup·energy 표본에 포함되는 측정 결함을 발견했다. 진단 로그의 직접 원인은 `pgrep -fl <battery path>` 결과 전체에 32개 상한을 먼저 적용한 것이었다. 성능 측정 셸처럼 battery CLI 경로를 인자에 포함한 무관한 프로세스가 56개 이상 잡히면 exact Maintain argv가 하나뿐이어도 `refusing unbounded process inspection`으로 preflight가 실패했다.

- 프로세스 조회 출력의 runner byte 상한은 유지하고, exact `/bin/bash <battery path> maintain_synchronous <target>` argv를 파싱한 뒤에만 32개 fan-out 상한을 적용한다. PID 제외, identity-before/`ps` argv/identity-after 검증과 duplicate-worker fail-closed 계약은 바꾸지 않았다.
- battery 경로를 포함한 무관한 80개 명령은 무시하면서 exact worker 하나를 선택하는 테스트와 exact worker 33개는 계속 거절하는 테스트를 추가했다.
- 실패한 최초 10회 비교는 성능 증거에서 폐기했다. 이후 동일한 80% Maintain 상태, 20초 준비 시간, 120초 창, background child activity가 0이 아님을 gate로 한 교차 4+4회 비교를 사용했다.
...
PR #18 병합 뒤 설치한 Release 앱을 battery CLI 경로를 포함하는 무관한 process 40개와 exact worker 1개, 즉 raw 후보 41개가 보이는 상태에서 재시작했다. 초기화가 정상 완료됐고 최종 상태는 80%, charging disabled, not discharging, Maintain 80, exact worker 1개였다. 체크포인트 20의 실기 gate를 통과했다.

### 0.27 PR #19 activation policy 진단 노이즈 조사 (2026-08-09)

`Diagnostics.json`에는 앱 시작마다 `NSApplication rejected activation policy 1`이 누적돼 조사 시점에 49건이었다. 이는 실제 accessory 정책 실패가 아니라 상태 확인 없이 idempotent setter를 호출하고 반환값만 해석한 거짓 실패였다.

- 앱은 `LSUIElement=true`라 agent app으로 시작하며, 설치 실행본의 `NSRunningApplication.activationPolicy`도 raw value 1인 accessory였다.
- `applicationDidFinishLaunching`이 같은 `.accessory`를 다시 요청했다. `setActivationPolicy`는 정책 switch가 성공했을 때만 true를 반환하므로 이미 적용된 정책에 대한 false를 코드가 실패로 기록했다.
...
26. `[구현·자동 검증 완료]` 물리 연결 evidence와 providing source를 typed observation으로 조합하고, generation-owned settlement/trailing read 및 순수 `BatteryPresentation`으로 세 UI surface를 통일했다. 전체 344개 테스트와 strict/Release/Analyze gate가 통과했으며 물리적 unplug/replug는 별도 read-only 확인으로 남긴다.
...
| Eng Review | `/plan-eng-review` | Architecture & tests (required) | 1 | clean | SMC sample lifetime and test topology were completed in PR #12 and PR #13; the remaining controller/SMC file decomposition was completed in PR #26 and PR #27 |
