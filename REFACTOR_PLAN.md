...
19. `[독립 성능 PR 구현·자동·실기 검증 완료, merge 대기]` 실제 Mac의 `flt` 온도 키를 누락하는 upstream `smc -t` 한계를 앱 번들 내부의 read-only `BatteryGuardSMCReader`로 제거했다. helper는 write API 없이 AppleSMC 연결 하나에서 `TB0T/TB1T/TB2T`만 직접 읽고, 세 키 전체가 유효할 때만 결과를 채택한다. 실행 파일 위치·형식·권한·identity를 검증하고 incomplete/fast failure는 기존 external batch와 bounded per-key 경로로 폴백하며 helper timeout/cancellation은 즉시 fail closed 한다. 호환 per-key 경로도 세 개의 개별 2초 timeout을 합산하지 않고 4.5초 aggregate budget 안에서 종료한다. 전체 키를 읽는 `smc -l`은 한 번 실행에 약 0.63초가 걸려 기존 세 직접 조회 약 0.01초보다 느리므로 거절했다. 최종 Release helper 20회 실기 비교에서 단일 sample 평균은 약 8.30ms, 기존 세 subprocess는 약 22.57ms였고 process 수는 sample당 3개에서 1개로 줄었다. helper의 Xcode coverage instrumentation이 stderr와 diagnostics를 오염시키는 실기 결함을 발견해 target-local disable flag로 제거했다. GPL-2.0-or-later helper는 별도 실행 파일·소스·번들 license로 MIT 앱과 경계를 유지한다. helper success/incomplete/fast-failure/missing-preflight/timeout/cancellation 및 aggregate deadline을 포함한 엄격 동시성·경고 오류화 조건의 272개 테스트, Release build와 Analyze가 통과했으며 검증 전후 실제 제어 상태는 80% Maintain, non-discharge로 동일했다.
