...
21. `[PR #19 구현·자동 검증 완료, 설치본 검증 예정]` `LSUIElement`로 이미 accessory인 launch를 idempotent no-op으로 처리하고, 실제 activation policy 전환은 setter 승인과 exact postcondition을 모두 검증해 거짓 실패 진단을 제거한다.
